...
class NPCAgent {
    ...
    async updateEmotions() {
        ...
        updates.emotionDesperation = Math.floor((npc.emotionFear + npc.emotionSadness + (100 - avgNeeds)) / 3);
        updates.emotionGrief = Math.floor((npc.emotionSadness * 2 + (100 - npc.needSocial)) / 3);
        await this.db.nPC.update({
            where: { id: this.id },
            data: updates
        });
     ...
     * Execute an action
     */
    async executeAction(action, day, hour) {
        const npc = await this.db.nPC.findUnique({ where: { id: this.id } });
        if (!npc)
            return;
    ...
    async addMemory(memory) {
        await this.db.memory.create({
            data: {
                ...
                involvedNpcs: (0, database_1.safeJsonStringify)(memory.involvedNpcs)
            }
        });
    }
    /**
