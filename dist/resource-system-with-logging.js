...
class ResourceManager {
    ...
    startProduction(npcName, occupation, currentHour, storageId) {
        ...
        // Check if has recipe
        const recipe = exports.PRODUCTION_RECIPES[occupationLower];
        if (!recipe) {
            console.log(`   ❌ No recipe for occupation "${occupation}"`);
            console.log(`   Available occupations:`, Object.keys(exports.PRODUCTION_RECIPES).join(", "));
            return null;
        }
        console.log(`   📋 Recipe: Can make ${recipe.amount}x ${recipe.produces} in ${recipe.timeHours}h`);
        // Check if already working
        const existing = this.productionTasks.find((t) => t.npcName === npcName && !t.completed);
        if (existing) {
            const timeLeft = existing.endHour - currentHour;
            console.log(`   ⏳ Already working! Making ${existing.amount}x ${existing.resource} (${timeLeft}h left)`);
            return existing;
        }
        ...
        if (recipe.requires && recipe.requires.length > 0) {
            ...
            for (const req of recipe.requires) {
                ...
                const status = has ? "✅" : "❌";
                console.log(`      ${status} ${req.resource}: need ${req.amount}, have ${available}`);
                if (!has)
        ...
        if (storageId) {
            ...
            if (storage) {
                ...
                if (spaceAvailable < recipe.amount) {
                    console.log(`   ⚠️  Storage almost full! Only ${spaceAvailable} space left.`);
                }
            }
        }
        // Create task
        ...
        console.log(`   ✅ Started! Will complete at hour ${task.endHour} (in ${recipe.timeHours}h)`);
        return task;
    }
    /**
    ...
    updateProduction(currentHour) {
        ...
        for (const task of this.productionTasks) {
            ...
            if (currentHour >= task.endHour) {
                ...
                console.log(`      Made: ${task.amount}x ${task.resource} ${exports.RESOURCE_INFO[task.resource].emoji}`);
                ...
                if (task.storageId) {
                    ...
                    if (storage) {
                        ...
                        if (added < task.amount) {
                            console.log(`      ⚠️  Storage full! Only added ${added}/${task.amount} (lost ${task.amount - added})`);
                        }
                        else {
                            console.log(`      ✅ Added to ${storage.name}`);
                            console.log(`      📦 ${storage.name} now has ${storage.get(task.resource)}x ${task.resource}`);
                        }
                    }
                    else {
                        console.log(`      ❌ Storage "${task.storageId}" not found! Resources lost!`);
                    }
                }
                else {
                    console.log(`      ⚠️  No storage specified - resources vanished into the void`);
                }
                completed.push(task);
            }
            else {
                // Still working
                console.log(`   ⏳ ${task.npcName}: Making ${task.resource} (${timeLeft}h left)`);
            }
        }
    ...
    logStorageState() {
        ...
        for (const storage of this.storages.values()) {
            ...
            console.log(`   Capacity: ${storage.getTotalStored()}/${storage.capacity} (${storage
                .getFillPercent()
                .toFixed(1)}% full)`);
            if (storage.inventory.size === 0) {
                ...
                for (const [resource, amount] of storage.inventory.entries()) {
                    const info = exports.RESOURCE_INFO[resource];
     ...
     */
    logNPCThoughts(npcs, currentHour) {
        console.log(`\n🧠 === NPC THOUGHTS (Hour ${currentHour}) ===`);
        for (const npc of npcs) {
            const task = this.getNPCTask(npc.name);
            const recipe = exports.PRODUCTION_RECIPES[npc.occupation?.toLowerCase()];
            console.log(`\n${npc.name} (${npc.occupation}):`);
            if (!recipe) {
                console.log(`   💭 "I don't know how to work... no recipe for my occupation"`);
                continue;
            }
            if (task) {
                const timeLeft = task.endHour - currentHour;
                console.log(`   💼 "I'm working on ${task.amount}x ${task.resource}... ${timeLeft}h to go!"`);
            }
            else {
                console.log(`   😴 "I'm idle. I could make ${recipe.produces} if I had materials..."`);
                if (recipe.requires) {
                    ...
                    if (storage) {
                        ...
                        for (const req of recipe.requires) {
                            ...
                            if (has >= req.amount) {
                                console.log(`      ✅ ${req.resource}: need ${req.amount}, have ${has}`);
                            }
                            else {
                                console.log(`      ❌ ${req.resource}: need ${req.amount}, only have ${has}`);
                                canWork = false;
                            }
                        }
                        if (canWork) {
...
//# sourceMappingURL=resource-system-with-logging.js.map