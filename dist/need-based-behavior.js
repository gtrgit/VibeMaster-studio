...
exports.NeedBasedBehavior = void 0;
/**
 * CRITICAL THRESHOLDS
 */
const THRESHOLDS = {
 ...
 */
class NeedBasedBehavior {
    /**
     * Main decision function - determines what NPC should do
     */
    decideAction(npc, currentHour) {
        console.log(`\n🧠 ${npc.name} is deciding what to do...`);
        console.log(`   Needs: Food(${npc.needFood}) Safety(${npc.needSafety}) Wealth(${npc.needWealth}) Social(${npc.needSocial}) Rest(${npc.needRest})`);
        ...
        if (npc.needSocial < THRESHOLDS.SOCIAL_LOW) {
            ...
            const location = npc.needSafety < THRESHOLDS.SECURITY_LOW ? npc.home : "tavern";
            options.push({
                location,
    ...
    decayNeeds(npc, hours) {
        ...
        console.log(`⏬ Needs decayed for ${npc.name}:`, `Food(${npc.needFood}) Safety(${npc.needSafety}) Wealth(${npc.needWealth}) Social(${npc.needSocial}) Rest(${npc.needRest})`);
    }
    /**
     * Recover needs based on current activity
     */
    recoverNeeds(npc, activity) {
        const recovery = {
        ...
        for (const [need, amount] of Object.entries(changes)) {
            const needKey = `need${need.charAt(0).toUpperCase() + need.slice(1)}`;
            const current = npc[needKey];
            npc[needKey] = Math.min(100, Math.max(0, current + amount));
        }
     ...
     */
    getActivityDescription(npc, location) {
...
exports.NeedBasedBehavior = NeedBasedBehavior;
//# sourceMappingURL=need-based-behavior.js.map