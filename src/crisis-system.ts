...
export function detectCrisis(npc: any): Crisis | null {
  ...
  if (npc.emotionHappiness < 15) {
    return {
      type: 'emotional_breakdown',
      severity: 'high',
      affectsDialogue: true,
      urgency: 85 - npc.emotionHappiness
    };
  }
...
export function getCrisisGreeting(npc: any, crisis: Crisis): string {
  const name = npc.name;
...
export function getCrisisWellbeingResponse(npc: any, crisis: Crisis): string {
  switch (crisis.type) {
    case 'starvation':
      if (crisis.severity === 'extreme') {
...
export function getCrisisOccupationResponse(npc: any, crisis: Crisis): string {
...
export function shouldInterruptForCrisis(crisis: Crisis | null): boolean {
  if (!crisis) return false;
  return crisis.severity === 'extreme' || crisis.urgency > 80;
}

// Get crisis-specific dialogue options
export function getCrisisDialogueOptions(npc: any, crisis: Crisis): Array<{text: string, action: string}> {
  const options: Array<{text: string, action: string}> = [];
