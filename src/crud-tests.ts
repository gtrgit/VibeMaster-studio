...
export async function runCRUDTests(): Promise<TestResult[]> {
  ...
  results.push(await runTest('Update World', async () => {
    ...
    if (!world) throw new Error('World not found for update');
  ...
  results.push(await runTest('Create NPC with Relations', async () => {
    ...
    if (!world || !location) throw new Error('Prerequisites not found');
  ...
  results.push(await runTest('Complex Query with Relations', async () => {
    const worldWithRelations = await prisma.world.findFirst({
      where: { name: 'CRUD Test World' },
      include: {
        locations: true,
        npcs: {
    ...
    if (!worldWithRelations) throw new Error('World not found');
    if (worldWithRelations.locations.length === 0) throw new Error('No locations found');
    if (worldWithRelations.npcs.length === 0) throw new Error('No NPCs found');
    if (!worldWithRelations.npcs[0].location) throw new Error('NPC location relation failed');
  }));
  ...
  results.push(await runTest('Clean World Deletion', async () => {
    ...
    await prisma.memory.deleteMany({ where: { npc: { worldId: world.id } } });
    await prisma.goal.deleteMany({ where: { npc: { worldId: world.id } } });
    await prisma.relationship.deleteMany({ where: { fromNpc: { worldId: world.id } } });
    await prisma.event.deleteMany({ where: { worldId: world.id } });
    await prisma.schedule.deleteMany({ where: { npc: { worldId: world.id } } });
    await prisma.nPC.deleteMany({ where: { worldId: world.id } });
    await prisma.location.deleteMany({ where: { worldId: world.id } });
    await prisma.world.delete({ where: { id: world.id } });

    // Verify deletion
    const remainingWorld = await prisma.world.findUnique({
      where: { id: world.id }
    });
    if (remainingWorld) {
...
export async function runPerformanceTests(): Promise<TestResult[]> {
  ...
  results.push(await runTest('Bulk Insert 100 NPCs', async () => {
    ...
    const created = await prisma.nPC.createMany({
      data: npcData
    });
  ...
  results.push(await runTest('Aggregate NPC Statistics', async () => {
    const stats = await prisma.nPC.aggregate({
  ...
  results.push(await runTest('Find Extroverted NPCs', async () => {
    const extroverts = await prisma.nPC.findMany({
      where: {
        worldId: world.id,
        extraversion: { gte: 60 }
      },
